...
class SingleInceptionCRNN_v2(nn.Module):
    def __init__(self, obs_dim, fnl_dim, num_dim, lag, dropout, added_point, rnn_type, reg):
        super(SingleInceptionCRNN_v2, self).__init__()
        obs_time = lag * 4
        fnl_time = lag * 4 - 2
        ...
        self.num_rnn = getattr(nn, rnn_type)(num_dim, num_dim, 1, True, batch_first=True)

        # self.num_inception = Inception(num_dim, 64, 96, 128, 16, 32, 32)
        # self.num_inception_dropout = nn.Dropout(dropout)

        self.fnl_resize = BasicConv1d(fnl_time, obs_time, kernel_size=1)
        self.fnl_resize_dropout = nn.Dropout(dropout)

        self.rnn_layer = getattr(nn, rnn_type)(512, 256, 1, True)
        ...
        self.fc1 = BasicFC(256 * (lag * 4) + num_fc_dim, 256)
    ...
    def forward(self, x_obs, x_fnl, x_num=None, point_num=None):
        ...
        x_fnl = self.fnl_inception_dropout(x_fnl)
        ...
        # rnn
        x = x.transpose(1, 2)
        x, hn = self.rnn_layer(x)

        x = x.view(x.shape[0], -1)

        # flattening numeric data and concat
        if x_num is not None:
            #             x_num = self.num_inception(x_num)
            #             x_num = self.num_inception_dropout(x_num)
...
class DoubleInceptionCRNN_v2(nn.Module):
    def __init__(self, obs_dim, fnl_dim, num_dim, lag, dropout, added_point, rnn_type, reg):
        super(DoubleInceptionCRNN_v2, self).__init__()
        obs_time = lag * 4
        fnl_time = lag * 4 - 2
    ...
    def forward(self, x_obs, x_fnl, x_num=None, point_num=None):
        ...
        if x_num is not None:
            ...
            _, out = self.num_rnn(x_num)
            out = out.squeeze()
            # print(out.shape, point_num.shape)
            if point_num is not None:
                out = torch.cat((out, point_num), dim=1)
            x = torch.cat((x, out), dim=1)
